...
const badgeVariants = cva(
  'inline-flex items-center min-h-[16px] min-w-[16px] px-2 py-0.5 border rounded-full font-medium text-xs text-white',
  {
    variants: {
      variant: {
...
const Badge: React.FC<BadgeProps> = ({
  ...
  <span
    className={cn(
      badgeVariants({ variant, absolute, left, right, top, bottom }),
      className,
    )}
    {...props}
  />
