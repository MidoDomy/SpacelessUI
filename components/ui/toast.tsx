...
>(({ className, ...props }, ref) => (
  <ToastPrimitives.Viewport
    ref={ref}
    className={cn(
      'fixed top-0 z-[100] flex flex-col gap-3 max-h-screen w-full p-5 sm:right-0 md:max-w-[420px]',
      className,
    )}
    {...props}
  />
...
const toastVariants = cva(
  'relative flex gap-3 items-start w-full overflow-hidden border-l-[3px] py-3 px-4 text-sm transition-all data-[swipe=cancel]:translate-x-0 data-[swipe=end]:translate-x-[var(--radix-toast-swipe-end-x)] data-[swipe=move]:translate-x-[var(--radix-toast-swipe-move-x)] data-[swipe=move]:transition-none data-[state=open]:animate-in data-[state=closed]:animate-out data-[swipe=end]:animate-out data-[state=closed]:fade-out-80 data-[state=closed]:slide-out-to-right-full data-[state=open]:slide-in-from-top-full data-[state=open]:sm:slide-in-from-bottom-full',
  {
    variants: {
      variant: {
        primary: 'border-primary-600 bg-primary-50 [&>.icon]:text-primary-600',
        secondary:
          'border-secondary-600 bg-secondary-50 [&>.icon]:text-secondary-600',
